...
with open('fan_data_threshold.csv') as csv_file:
    ...
    for row in csv_reader:
        ...
        if(old_light_value == 1 and current_light_value == 0):
          pulse_start = float(row[0])
          blade_count += 1
          
          
        if(old_light_value == 0 and current_light_value == 1):
          pulse_end = float(row[0])
          pulse_data.append([row[0],pulse_end-pulse_start])
...
f = open("pulse_data_output.csv", "w")
